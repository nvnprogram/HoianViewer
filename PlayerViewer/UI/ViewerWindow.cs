...
namespace PlayerViewer.UI
{
    ...
    public class ViewerWindow : GameWindow
    {
        ...
        void DrawPlayerPanel()
        {
            ...
            if (ImGui.RadioButton("Alpha", _teamIndex == 0)) { _teamIndex = 0; ApplyTeamColor(); }
            ImGui.SameLine();
            if (ImGui.RadioButton("Bravo", _teamIndex == 1)) { _teamIndex = 1; ApplyTeamColor(); }
            ImGui.SameLine();
            if (ImGui.RadioButton("Charlie", _teamIndex == 2)) { _teamIndex = 2; ApplyTeamColor(); }

            Widgets.SectionHeader("Gear");
            //No "Blank" for head: Hed_INV000 (first entry) is the game's real
            //no-headgear and carries the default hair-arrange presets.
        ...
        void DrawViewSection()
        {
            ...
            //Reset must re-frame the ACTIVE scene: framing the player while a big
            //standalone model is open also shrinks the self-shadow light bounds to
            //player size, killing the model's shadows.
            ...
            //Which dumped uniform set drives the lighting (gear-viewer vs in-stage).
            ...
            Widgets.LabeledRow("Env", () =>
            {
                ...
                if (ImGui.Combo("##uniset", ref setIdx, UniformSetLabels, UniformSetLabels.Length))
                {
                    ...
                    //The reload resets team colors to the dump's defaults.
                    ApplyTeamColor();
                }
            });
        }
        ...
        void DrawCapturePanel()
        {
            ...
            if (!_recorder.IsRecording)
            {
                ...
                //Recording requires ffmpeg (PNG-sequence fallback was removed).
