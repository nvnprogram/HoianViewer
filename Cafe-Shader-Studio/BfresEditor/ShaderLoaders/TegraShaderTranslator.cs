...
namespace BfresEditor
{
    public class TegraShaderTranslator
    {
        ...
        /// <summary>
        /// Forces all lazy GL capability queries to evaluate now. Must be called on
        /// a thread with a current GL context before Translate is used off-thread.
        /// </summary>
        public static void InitCaps()
        {
            _ = _supportsFragmentShaderInterlock.Value;
            _ = _supportsFragmentShaderOrdering.Value;
            _ = _supportsShaderBallot.Value;
            _ = _supportsImageLoadFormatted.Value;
            _ = _supportsTextureShadowLod.Value;
            _ = _maximumComputeSharedMemorySize.Value;
            _ = _storageBufferOffsetAlignment.Value;
            _ = _gpuVendor.Value;
        }
