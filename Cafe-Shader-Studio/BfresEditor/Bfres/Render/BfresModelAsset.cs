...
namespace BfresEditor
{
    public class BfresModelAsset : ModelAsset, IPickable
    {
        ...
        static Type[] BfshaRenders = new Type[]
        {
            ...
            typeof(BlitzNXRender), typeof(HoianNXRender),
            typeof(KSANXRender),
        ...
        public BfresModelAsset(BFRES bfres, BfresRender render, FMDL model) {
            ...
            var swMesh = new System.Diagnostics.Stopwatch();
            var swShader = new System.Diagnostics.Stopwatch();
            ...
            Console.WriteLine($"[BfresModelAsset] {model.Name}: mesh assets {swMesh.Elapsed.TotalSeconds:F2}s, shader lookup {swShader.Elapsed.TotalSeconds:F2}s");
        ...
        private void DrawCustomShaderRender(GLContext control, BfresMeshAsset mesh, int stage = 0)
        {
            ...
            if (materialAsset.Shader.IsPending && !materialAsset.Shader.PollReady())
                return;

            if (TegraShaderDecoder.AllowDeferredCompile &&
                !materialAsset.FirstRenderDone && !ShaderRenderBase.TryClaimFirstRenderSlot())
                return;
