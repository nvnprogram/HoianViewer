...
{
    public class BntxTexture : STGenericTexture, IDragDropNode, GLFrameworkEngine.IMipSurfaceProvider
    {
        /// <summary>
        ...
        {
            TexFormat.BGR5A1_UNORM,
            TexFormat.BGRA4_UNORM,
            TexFormat.BGR5A1_UNORM,
            TexFormat.BGR565_UNORM,
            TexFormat.BGRA8_SRGB,
            TexFormat.BGRA4_UNORM,
            TexFormat.RGBB10A2_UNORM,
            TexFormat.R16_UNORM,
            TexFormat.RGBA4_UNORM,
            TexFormat.RG4_UNORM,
            TexFormat.RGB5A1_UNORM,
            TexFormat.RG8_UNORM,
            TexFormat.R8_UNORM,
        ...
        /// <summary>
        /// Deswizzles a single mip level directly from the BNTX per-mip data.
        /// The parser stores each mip's swizzled data starting at TextureData[array][mip],
        /// so each level can be deswizzled independently with its own block height.
        /// </summary>
        public byte[] GetMipSurface(int arrayLevel, int mipLevel)
        {
            try
            {
                if (Texture.TileMode != TileMode.Default)
                    return null; //Pitch linear, use the default path.
                if (arrayLevel >= Texture.TextureData.Count ||
                    mipLevel >= Texture.TextureData[arrayLevel].Count)
                    return null;

                var mipData = Texture.TextureData[arrayLevel][mipLevel];

                uint blkWidth = GetBlockWidth();
                uint blkHeight = GetBlockHeight();
                uint bpp = GetBytesPerPixel();

                uint mipWidth = Math.Max(1u, Width >> mipLevel);
                uint mipHeight = Math.Max(1u, Height >> mipLevel);
                uint widthInBlocks = TegraX1Swizzle.DIV_ROUND_UP(mipWidth, blkWidth);
                uint heightInBlocks = TegraX1Swizzle.DIV_ROUND_UP(mipHeight, blkHeight);

                //Per-mip block height (Tegra TRM): halve while the mip fits in half the GOB rows.
                uint mipBlockHeight = 1u << (int)Texture.BlockHeightLog2;
                while (heightInBlocks <= (mipBlockHeight / 2) * 8 && mipBlockHeight > 1)
                    mipBlockHeight /= 2;
                int mipBlockHeightLog2 = (int)Math.Log(mipBlockHeight, 2);

                var deswizzled = TegraX1Swizzle.deswizzle(
                    mipWidth, mipHeight, 1, blkWidth, blkHeight, 1,
                    1, bpp, 0, mipBlockHeightLog2, mipData);

                uint outputSize = widthInBlocks * heightInBlocks * bpp;
                if (deswizzled.Length < outputSize)
                    return null;
