...
namespace CafeShaderStudio
{
    public class MainWindow : GameWindow
    {
        ...
        public static string PendingAutoLoadFile;
            ...
            var Thread = new Thread((ThreadStart)(() =>
            {
                ...
                foreach (var file in Directory.GetFiles("GlobalShaders"))
                {
                    if (!GlobalShaderCache.ShaderFiles.ContainsKey(file))
                    {
                        ...
                        else if (file.EndsWith(".bfsha.zs"))
                        {
                            var decompressed = CompressionLibrary.Zstb.SDecompress(File.ReadAllBytes(file));
                            GlobalShaderCache.ShaderFiles.Add(file, new BfshaLibrary.BfshaFile(new MemoryStream(decompressed)));
                        }
                        else if (file.EndsWith(".sharcfb"))
        ...
        public void LoadFileFormat(string fileName)
        {
            ...
            {
                //Zstd compressed file (ie Splatoon 3 romfs). Decompress in memory and
                //keep the original path so game data next to the file can be located.
                var decompressed = CompressionLibrary.Zstb.SDecompress(System.IO.File.ReadAllBytes(fileName));
                string fakePath = fileName.Substring(0, fileName.Length - 3);
                using (var ms = new System.IO.MemoryStream(decompressed))
                    fileFormat = STFileLoader.OpenFileFormat(ms, fakePath);
            }
            else
            {
                fileFormat = STFileLoader.OpenFileFormat(fileName);
            }
            if (fileFormat == null)
            {
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            if (PendingAutoLoadFile != null && initGlobalShaders)
            {
                string autoFile = PendingAutoLoadFile;
                PendingAutoLoadFile = null;
                try
                {
                    LoadFileFormat(autoFile);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Failed to load {autoFile}: {ex.Message}");
                }
            }
        ...
        private void DrawSplatoon3Window()
        {
            ImGui.SetNextWindowSize(new System.Numerics.Vector2(340, 220), ImGuiCond.FirstUseEver);
            if (ImGui.Begin("Splatoon 3"))
            {
                ImGui.Text("Team Colors");
                ImGui.Separator();

                var alpha = HoianNXRender.TeamAlphaColor;
                if (ImGui.ColorEdit3("Alpha (My Team)", ref alpha,
                    ImGuiColorEditFlags.Float | ImGuiColorEditFlags.HDR))
                    HoianNXRender.TeamAlphaColor = alpha;

                var bravo = HoianNXRender.TeamBravoColor;
                if (ImGui.ColorEdit3("Bravo", ref bravo,
                    ImGuiColorEditFlags.Float | ImGuiColorEditFlags.HDR))
                    HoianNXRender.TeamBravoColor = bravo;

                var charlie = HoianNXRender.TeamCharlieColor;
                if (ImGui.ColorEdit3("Charlie", ref charlie,
                    ImGuiColorEditFlags.Float | ImGuiColorEditFlags.HDR))
                    HoianNXRender.TeamCharlieColor = charlie;

                ImGui.Spacing();
                if (ImGui.Button("Reset to Game Defaults"))
                    HoianNXRender.ResetTeamColors();
            }
            ImGui.End();
        }
